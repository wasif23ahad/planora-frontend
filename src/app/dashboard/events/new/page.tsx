...
export default function CreateEventPage() {
  ...
  const onSubmit = async (data: EventForm) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || "Failed to create event. Check your inputs.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-[800px] mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-10 bg-white border border-border-base p-10 rounded-2xl shadow-sm">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex-1 space-y-2">
              ...
              />
              <p className="text-[12px] text-muted italic">Recommended: 800x600px. JPG, PNG or WebP.</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Input label="Event Title" placeholder="e.g. AI Masterclass 2026" {...register("title")} error={errors.title?.message} />
          <Input label="Venue" placeholder="e.g. Grand Ballroom, Westin" {...register("venue")} error={errors.venue?.message} />
